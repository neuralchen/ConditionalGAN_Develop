...
def prepare_inception_metrics(dataset, sampler,num_inception_images, parallel=False):
  ...
  data_mu     = np.load(dataset+'inception_moments.npz')['mu']
  data_sigma  = np.load(dataset+'inception_moments.npz')['sigma']
  # Load network
...
def prepare_dataset_inception_moments(dataRoot,savePath,dataName="cifar10"):
  ...
  if dataName == "cifar10":
    ...
    trainset = torchvision.datasets.CIFAR100(root=dataRoot+'/cifar100', train=True,
                                          download=True, transform=transform)
  ...
  print(sigma)
  print(sigma.shape)
  if not os.path.exists(os.path.join(savePath, dataName)):
    os.makedirs(os.path.join(savePath, dataName))
  np.savez(savePath+'/%s/C100_inception_moments.npz'%dataName, mu=mu, sigma=sigma)
...
if __name__ == "__main__":
    prepare_dataset_inception_moments("./data",'./datasetMoment',"cifar100")