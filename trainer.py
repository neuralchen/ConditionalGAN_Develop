...
# Last Modified:  Sunday, 6th October 2019 8:03:19 pm
...
class Trainer(object):
    ...
    def train(self):
        ...
        sampleBatch = 10
        SampleClass = 10
        fixed_z = torch.randn(SampleClass*sampleBatch, self.z_dim)
        fixed_z = fixed_z.to(self.device)
        fixed_c = Sampler.sampleFixedLabels(SampleClass,sampleBatch,self.device)
        runingZ,runingLabel = Sampler.prepare_z_c(self.batch_size, self.z_dim, SampleClass, device=self.device)
        ...
        for step in range(start, self.total_step):
            ...
            if (step + 1) % self.sample_step == 0:
                ...
                save_image(denorm(fake_images.data),
                           os.path.join(self.sample_path, '{}_fake.png'.format(step + 1)),nrow=SampleClass)
